"""pytest plugin — captures YouTube test outcomes and generates an HTML report.

Hooks fire for every test in the session but only youtube node IDs are
stored. At session end the plugin writes:

  $NOTEBOOKLM_ARTIFACTS/youtube_report.html
"""
import os
import re
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when != "call" or "youtube" not in item.nodeid:
        return
...
def pytest_sessionfinish(session, exitstatus) -> None:
    ...
    out = artifacts_dir / "youtube_report.html"
    _write_html(out)
    print(f"\nHTML report → {out}")
...
_TC_LABELS = {
    "1":  "Manual Captions (Precise)",
    "2":  "Auto Captions (Medium Quality)",
    "3":  "Low Quality Captions",
    "4":  "No Captions Available",
    "5":  "Invalid URL",
    "6":  "Restricted Video",
    "7":  "Mixed Language Captions",
    "8":  "Unsupported Language",
    "9":  "Compare Query",
    "10": "Quiz Generation",
    "11": "Out-of-Scope Query",
    "12": "Complex Explanation",
    "13": "Manual Captions (Variant)",
    "14": "Auto Captions (Variant Query)",
    "15": "Low Quality Captions (Variant)",
    "16": "No Captions Check",
    "17": "Invalid URL Check",
    "18": "Mixed Language (Variant Query)",
    "19": "Multi-Citation Check",
    "20": "Long Complex Query",
}
...
<head>
  ...
  <title>YouTube · NotebookLM Test Report</title>
  <style>{_CSS}</style>
</head>
<body>
  <h1>YouTube &mdash; NotebookLM Test Report</h1>
