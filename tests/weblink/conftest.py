"""pytest plugin — captures weblink test outcomes and generates an HTML report.

Hooks fire for every test in the session but only weblink node IDs are
stored. At session end the plugin writes:

  $NOTEBOOKLM_ARTIFACTS/weblink_report.html
"""
import os
import re
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when != "call" or "weblink" not in item.nodeid:
        return
...
def pytest_sessionfinish(session, exitstatus) -> None:
    ...
    out = artifacts_dir / "weblink_report.html"
    _write_html(out)
    print(f"\nHTML report → {out}")
...
_TC_LABELS = {
    "1":  "No Source Handling",
    "2":  "Single Source (Precise)",
    "3":  "Multi-Source",
    "4":  "Restricted Link",
    "5":  "Invalid URL",
...
<head>
  ...
  <title>Weblink · NotebookLM Test Report</title>
  <style>{_CSS}</style>
</head>
<body>
  <h1>Weblink &mdash; NotebookLM Test Report</h1>
