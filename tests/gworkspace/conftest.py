"""pytest plugin — captures gworkspace test outcomes and generates an HTML report.

Hooks fire for every test in the session but only gworkspace node IDs are
stored. At session end the plugin writes:

  $NOTEBOOKLM_ARTIFACTS/gworkspace_report.html
"""
import os
import re
...
def pytest_runtest_makereport(item, call):
    ...
    # Only care about the actual test body, and only gworkspace tests
    if report.when != "call" or "gworkspace" not in item.nodeid:
        return

    if report.outcome == "passed":
        status, reason = "PASS", ""
    elif report.outcome == "skipped" and hasattr(report, "wasxfail"):
        # pytest.xfail() called inside the test — we surface this as FAIL
        status = "FAIL"
...
def pytest_sessionfinish(session, exitstatus) -> None:
    ...
    out = artifacts_dir / "gworkspace_report.html"
    _write_html(out)
    print(f"\nHTML report → {out}")
...
_TC_LABELS = {
    ...
    "3":  "Unclear Query",
    "4":  "Summarization",
    "5":  "Multi-Source Summary",
    "6":  "Comparison",
    "7":  "Table Query (Full)",
    "8":  "Table Query (Partial)",
    "9":  "Table Query (N/A)",
    "10": "Restricted Source",
}
...
<head>
  ...
  <title>Google Workspace · NotebookLM Test Report</title>
  <style>{_CSS}</style>
</head>
<body>
  <h1>Google Workspace &mdash; NotebookLM Test Report</h1>
